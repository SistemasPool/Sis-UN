...
$cod_cliente = $_REQUEST['cod_cliente'];
                  ...
                  $select = $con ->query("SELECT * FROM automovil WHERE cod_cliente='$cod_cliente' AND nro_poliza='$num_poliza'");
		          $auto = $select -> fetch_assoc();
                  $fecha_pago = $auto['fechavencimiento']; 
...
<nav class="navbar navbar navbar-default navbar-static-top marginBottom-0" role="navigation">
    ...
    <div class="container">
      <div class="starter-template">
        ...
        <div class="row">
            <?php
             $estate = $con ->query("SELECT * FROM calculo_prima WHERE cod_cliente='$cod_cliente' AND nro_poliza='$num_poliza'");
            $tipo_pago;
         if($tipo_pago=='CONTADO'){
            ?>
    <h5>Prima Total</h5>
        ...
        <thead>
            <tr><th width="20%">Prima Total</th>
                ...
                <?php 
            if($esta_cuenta=='PAGADO' and $usuario=='Auditoria')   
            {
            ?>
        ...
        <tbody>
           <?php 
          		  $sel = $con ->query("SELECT SUM(prima_total) as prima_to, nro_poliza, estado, nro_factura, id_calculo_prima FROM calculo_prima WHERE cod_cliente='$cod_cliente' AND nro_poliza='$num_poliza' GROUP BY nro_poliza");
		          
                  
          while($estado_cuenta = $sel -> fetch_assoc()){
             //$esta_cuenta = $estate_count['estado'];
              $id = $estado_cuenta['id_calculo_prima'];
                  $prima_total = $estado_cuenta['prima_to'];
		 ...
		 <tr>
            ...
            <td>
            <?php 
            if($esta_cuenta=='PAGADO')   
            {
                ?>
             
            <?php
                }elseif($usuario=="Auditoria"){
            ...
            <button type="button" class="btn btn-success btn-xs" style="font-size:11px;" data-toggle="modal" data-target="#exampleModal" data-whatever="<?php echo $id; ?>" data-whateverpago="<?php echo $estado; ?>" data-whatevercliente="<?php echo $cod_cliente; ?>" data-whatevernumpoliza="<?php echo $num_poliza; ?>" data-whatevertipopago="<?php echo $tipo_pago; ?>" data-whateverfactura="">Actualizar</button>   
            <?php
                }
             ?>
            </td>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="actualiza_edo_cuenta.php" method="post">
...
<nav class="navbar navbar navbar-default navbar-static-top marginBottom-0" role="navigation">
        ...
        <li class="dropdown">
          ...
          <ul class="dropdown-menu">
            <li><a href="#"><?php echo $usuario = $_SESSION["usuario"]; ?></a></li>
            <li><a href="#"><?php echo $distrito = $_SESSION["distrito"]; ?></a></li>
            <li><a href="../../../salir.php">Salir</a></li>
          </ul>
        </li>
      </ul>
    ...
    <div class="container">
     ...
     <?php
            ...
            $cuota_inicial = $con ->query("SELECT SUM(monto) AS monto_inicial, id_cuo_ini, fecha_cuotas, estado, nro_factura FROM cuota_inicial WHERE cod_cliente='$cod_cliente' and num_poliza='$num_poliza' GROUP BY num_poliza, fecha_cuotas");
		    $cuoini = $cuota_inicial -> fetch_assoc();
            
            $cuotas = $con ->query("SELECT SUM(monto) as monto_cuotas,id_cuotas, num_poliza, fecha_cuotas, nro_factura, estado FROM cuotas WHERE num_poliza='$num_poliza' GROUP BY num_poliza, fecha_cuotas");
        ...
        <tbody>
		 <tr>
            <td><?php echo 'USD $'.$cuoini['monto_inicial']; ?></td>
            <td><?php echo $cuoini['fecha_cuotas'];?></td>
		 	<td><?php echo $cuoini['estado']; ?></td>
             <td><?php echo $cuoini['nro_factura']; ?></td>
             <td>
                <?php 
            $cuin = $con ->query("SELECT * FROM cuota_inicial WHERE cod_cliente='$cod_cliente' AND num_poliza='$num_poliza'");
		     $cu_in = $cuin -> fetch_assoc();
             $esta_cuin = $cu_in['estado'];
            if($esta_cuin=='PAGADO')   
            {
                ?>
            ...
            <?php
                ...
                ?>
                 <a href="Facturacion/nueva_factura.php?id=<?php echo $id;?>&cod_cliente=<?php echo $cod_cliente; ?>&nitf_cif=<?php echo $nitf_cif; ?>&pagador=<?php echo $pagador; ?>&nro_poliza=<?php echo $num_poliza; ?>&estado=<?php echo $cuoini['estado']; ?>&monto=<?php echo $cuoini['monto_inicial'];?>"><button class="btn btn-primary btn-xs" type="submit">Facturar</button></a>
                 <button type="button" class="btn btn-success btn-xs" style="font-size:11px;" data-toggle="modal" data-target="#exampleModal" data-whatever="<?php echo $cuoini['id_cuo_ini']; ?>" data-whateverpago="<?php echo $estado; ?>" data-whatevercliente="<?php echo $cod_cliente; ?>" data-whatevernumpoliza="<?php echo $num_poliza; ?>" data-whatevertipopago="<?php echo $tipo_pago; ?>" data-whateverfactura="" data-whatevercuotainicial="CUOTA INICIAL">Actualizar</button>
             <?php
                }
                ?>
                 </td>
  ...
  <div class="modal-dialog" role="document">
    ...
    <!-- final modal cuota inicial-->
        ...
        <tbody>
            ...
            ?>
		 <tr>     
            <td><?php echo 'USD $'.$cuotass['monto_cuotas']; ?></td>
            ...
            <?php
                ...
                ?>
            <a href="Facturacion/nueva_factura.php?id=<?php echo $id;?>&cod_cliente=<?php echo $cod_cliente; ?>&nitf_cif=<?php echo $nitf_cif; ?>&pagador=<?php echo $pagador; ?>&nro_poliza=<?php echo $num_poliza; ?>&estado=<?php echo $estado; ?>&monto=<?php echo $cuotass['monto_cuotas']; ?>"><button class="btn btn-primary btn-xs" type="submit">Facturar</button></a>
            <button type="button" class="btn btn-success btn-xs" style="font-size:11px;" data-toggle="modal" data-target="#exampleModalcuotas" data-whatever="<?php echo $cuotass['id_cuotas']; ?>" data-whateverpago="<?php echo $estado; ?>" data-whatevercliente="<?php echo $cod_cliente; ?>" data-whatevernumpoliza="<?php echo $num_poliza; ?>" data-whatevertipopago="<?php echo $tipo_pago; ?>" data-whateverfactura="" data-whatevercuotainicial="CUOTAS">Actualizar</button>
                  <?php } ?>    
             </td>
		 </tr>
        </tbody>

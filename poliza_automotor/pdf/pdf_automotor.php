...
?>
  <style>
    @page { margin: 37px 20px 50px 20px; }
    body { margin: 45px 20px 20px 20px; }
    table{
    ...
    <?php
      $sql = $con ->query("SELECT * FROM automovil where nro_poliza='$nro_poliza'");  
        ...
        {  
            ...
            <tr><td style="font-size:9px; text-align:center;">C&oacute;digo de Autorizaci&oacute;n: 116-910552-2018 02 003</td></tr>
            <?php
        }else
         {
             ?>
...
<td style="font-size:6.5px;"><?php echo utf8_decode($fila['direccion']); ?></td>
<td>Zona:</td>
...
<tr>
<td width="10%" style="font-size:9px;">Asegurado</td>
<td width="45%"></td>
<td width="15%"></td>
<td width="30%"></td>
</tr>
...
<tr>
<td>Direcci&oacute;n:</td>
<td style="font-size:6.5px;"><?php echo utf8_decode($tom['direccion']); ?></td>
<td>Zona:</td>
...
<tr>
<td>Direcci&oacute;n:</td>
<td style="font-size:6.5px;"><?php echo utf8_decode($ben['direccion']); ?></td>
<td>Zona:</td>
...
<table width=90% style="font-size:8px;" align="center" cellspacing=0 cellpadding=0>
    ...
    $fe_cuota = $con ->query("SELECT *, SUM(monto) as monto_cuota FROM cuotas WHERE cod_cliente='$cod_cliente' and num_poliza='$nro_poliza' group by fecha_cuotas");
    while($fecuo = $fe_cuota -> fetch_assoc()){

    
    $nueva_cuota = $fecuo['monto_cuota'];
...
<?php
if($nro_poliza=='AULP00000356' or $nro_poliza=='AULP00000357' or $nro_poliza=='AULP00000358'){
...
<?php
    $inter = $con ->query("SELECT intermediario FROM automovil WHERE nro_poliza='$nro_poliza'");
    ?>
<hr width=90%>
<table width=90% style="font-size:8px;" align="center" cellspacing=0 cellpadding=0>
    <tr>
    ...
    ?>   
        <tr>
        <td width=85%><?php echo $int['intermediario'];?></td>
        </tr>
    <?php } ?>
</table>
<hr width="90%">
<table width=90% style="font-size:8px;" align="center" cellspacing=0 cellpadding=0>
    ...
    <?php 
        if($_SESSION["distrito"]=="SANTA CRUZ"){
            $distrito = "Santa Cruz";
        }elseif($_SESSION["distrito"]=="LA PAZ"){
            $distrito = "La Paz";
        }elseif($_SESSION["distrito"]=="COCHABAMBA"){
            $distrito = "Cochabamba";
        }
    ?>    
    
    <td><?php echo $distrito." ,". date("d") . " de " . $mes . " del " . date("Y"); ?></td>  
    </tr> 
</table>
<br><br>
<table width=90% style="font-size:8px;" align="center" cellspacing=0 cellpadding=0>

...
$se = $con ->query("SELECT MAX(id_num) as idpoliza FROM acc_perso_poliza WHERE distrito='$s_distrito'");
		          $fila = $se -> fetch_assoc();
...
$nom_comp = $nombres_ct." ".$paterno_ct." ".$materno_ct;
...
$inc = $con -> query("SELECT * FROM acc_perso_cotizacion WHERE cod_cotizacion='$cod_cotizacion'");
...
$row = $inc -> fetch_assoc();
$valor_asegurado=$row['suma_asegurada'];
$intermediario=$row['intermediario'];
$prima_contado = $row['prima_total'];
...
$f_registro = date("Y-m-d");
                  ...
                  $porcentaje_utilidad = 12;
                    ...
                    $iva = $prima_contado*$porcentaje_iva/100;
                    $it = $prima_contado*$porcentaje_it/100;
                    $itf = $prima_contado*$porcentaje_itf/100;
                    $tasa_adicional = $prima_contado-$iva-$it-$itf;
                    ...
                    $prima_adicional = $prima_contado-$prima_tecnica;
...
$ins = $con ->query("INSERT INTO `calculo_prima`(`id_calculo_prima`, `modalidad`, `ramo`, `cod_cliente`, `nro_poliza`, `nombre_cliente`, `valor_asegurado`, `distrito`, `prima_riesgo`, `intermediario`, `gastos_admon`, `aps`, `fpa`, `remesas_exterior`, `utilidad`, `iva`, `it`, `itf`, `prima_total`, `prima_tecnica`, `prima_adiciona`, `tipo_pago`, `intermediario_prima`, `fecha_registro`, `certificado`, `estado`, `nro_factura`, `moneda`, `american`) VALUES ('','$modalidad','$ramo','$cod_cliente','$nro_poliza','$nom_comp','$valor_asegurado','$distrito','$prima_riesgo','','$gastos_admon','$aps','$fpa','$rem_ext','$utilidad','$iva','$it','$itf','$prima_contado','$prima_tecnica','$prima_adicional','CONTADO','','$f_registro','','NO PAGADO','','2','$porcentaje_ameass')");
    
$seco = $con ->query("SELECT * FROM acc_perso_cotizacion WHERE cod_cotizacion='$cod_cotizacion'");
		          $deco = $seco -> fetch_assoc();
                  $cod_cliente= $deco['cod_cliente'];
                  $num_poliza= $deco['num_poliza'];
                  $nombre_completo= $deco['nombre_completo'];
                  $suma_asegurada= $deco['suma_asegurada'];
                  $prima_total= $deco['prima_total'];
                  $prima_credito= $deco['prima_credito'];
                  $inicio_vigencia= $deco['inicio_vigencia'];
                  $final_vigencia= $deco['final_vigencia'];
                  $com_inter= $deco['com_inter'];
                  $intermediario= $deco['intermediario'];
                  $tipo_pago= $deco['tipo_pago'];
                  
        
$pol_accidentes = $con -> query("INSERT INTO `acc_perso_poliza`(`id_pol_ap`, `cod_cliente`, `num_poliza`, `id_num`, `nombre_completo`, `cod_cotizacion`, `suma_asegurada`, `prima_total`, `prima_credito`, `inicio_vigencia`, `final_vigencia`, `com_inter`, `intermediario`, `tipo_pago`, `f_registro`, `distrito`, `vencimiento`) VALUES ('','$cod_cliente','$nro_poliza','$idau','$nom_comp','$cod_cotizacion','$suma_asegurada','$prima_total','$prima_credito','$inicio_vigencia','$final_vigencia','$com_inter','$intermediario','$tipo_pago','$f_registro','$s_distrito', '$fecha_vencimiento')");
        
$beneficiarios = $con -> query("UPDATE beneficiarios_ap SET num_poliza='$nro_poliza' WHERE cod_cliente='$cod_cliente' and cod_cotizacion='$cod_cotizacion'");
        
$clausulas = $con -> query("UPDATE clau_accidentes_pol SET num_poliza='$nro_poliza' WHERE cod_cliente='$cod_cliente' and cod_cotizacion='$cod_cotizacion'");

}

if ($pol_accidentes) {
	header('Location: ../ver_poliza_accidentes.php');
}else{

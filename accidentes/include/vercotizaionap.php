    ...
    <div class="container">
      <div class="starter-template">
        <br><br>
		          ...
		          ?>
     <div>     
    <a href="pdf_ap_cot.php?cod_cotizacion=<?php echo $cod_cotizacion; ?>"><button class="btn btn-default pull-right" type="submit"><i class="far fa-file-pdf"></i>
</button></a>
    ...
    <!-- Inicio en Vivo  -->      
        ...
        <div class="row">
            ...
            <tr>
                ...
                <td>Invalidez</td>
                <td>Gastos M&eacute;dicos</td>
                <td></td>
            </tr>
            <?php 
            $coberturas = $con -> query("SELECT * FROM beneficiarios_ap WHERE cod_cotizacion='$cod_cotizacion'");
            while($cob = $coberturas -> fetch_assoc()){
            ?>  <tr>
                <td><?php echo utf8_decode(strtoupper($cob['nombre_completo']));?></td>
                <td><?php echo "USD ".$cob['muerte_accidental'];?></td>
                <td><?php echo "USD ".$cob['invalidez'];?></td>
                <td><?php echo "USD ".$cob['gastos'];?></td>
                <td><a href="../include/actualizar.php?id=<?php echo $cob['id_ben_ap'] ?>&cod_cliente=<?php echo $cob['cod_cliente'] ?>&cod_cotizacion=<?php echo $cob['cod_cotizacion'];?>"><button class="btn btn-success btn-xs" type="submit"><i class="fas fa-edit"></i></button></a>
                <a href="../include/borrar.php?id=<?php echo $cob['id_ben_ap'] ?>&cod_cliente=<?php echo $cob['cod_cliente'] ?>&cod_cotizacion=<?php echo $cob['cod_cotizacion'];?>"><button class="btn btn-danger btn-xs" onclick = "return confirm('¿Realmente desea eliminar?')" type="submit"><i class="fas fa-trash-alt"></i></button></a></td>
            </tr>
            <?php 
            }
            ?>
...
<table width=90% align="center" class="table table-striped">
        ...
        <!-- Modal -->
        <form action="../include/guardar.php" method="post">
        <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Beneficiario Accidentes Personales</h5>
              ...
              <div class="modal-body">
                ...
                <input type="hidden" value="<?php echo $_REQUEST['cod_cliente']; ?>" name="cod_cliente" id="cod_cliente">
                  ...
                  <div class="form-group">
                    <label for="formGroupExampleInput">Monto Muerte Accidental</label>
                    <input type="text" class="form-control" id="muerte" name="muerte">
                  </div>
                  <div class="form-group">
                    <label for="formGroupExampleInput">Monto Invalidez</label>
                    <input type="text" class="form-control" id="invalidez" name="invalidez">
                  </div>
                  <div class="form-group">
                    <label for="formGroupExampleInput">Monto Gastos M&eacute;dicos</label>
        ...
        <div class="row">
            ...
            <?php 
            $cla_adic = $con -> query("SELECT * FROM `clau_accidentes_pol` WHERE cod_cotizacion='$cod_cotizacion'");
            while($clau = $cla_adic -> fetch_assoc()){
            ?>
                <tr><td><?php echo utf8_decode(strtoupper($clau['glosa']));?></td>
                <td><a href="../clausulas/actualizar.php?id=<?php echo $clau['id_accid_pol']; ?>&cod_cliente=<?php echo $clau['cod_cliente'] ?>&cod_cotizacion=<?php echo $clau['cod_cotizacion'];?>"><button class="btn btn-success btn-xs" type="submit"><i class="fas fa-edit"></i></button></a>
            <a href="../clausulas/borrar.php?id=<?php echo $clau['id_accid_pol']; ?>&cod_cliente=<?php echo $clau['cod_cliente'] ?>&cod_cotizacion=<?php echo $clau['cod_cotizacion'];?>"><button class="btn btn-danger btn-xs" type="submit" onclick = "return confirm('¿Realmente desea eliminar?')"><i class="fas fa-trash-alt"></i></button></a></td>
                </tr>
            <?php 
            }
            ?>
